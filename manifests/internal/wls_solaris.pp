...
)
{
  ...
  if !defined(File[$fmw_wls::orainst_dir]) {
    file { $fmw_wls::orainst_dir:
      ensure => directory,
      before => File["${fmw_wls::orainst_dir}/oraInst.loc"],
      mode   => '0755',
    }
  }

  # add oraInst.loc to /var/opt/oracle for the oracle inventory location
  ...
  if ( $fmw_wls::version in ['10.3.6', '12.1.1'] ) {
    exec{ 'Install WLS':
      command     => "${java_home_dir}/bin/java -d64 -Xmx1024m -Djava.io.tmpdir=${fmw_wls::tmp_dir} -Duser.country=US -Duser.language=en -jar ${source_file} -mode=silent -silent_xml=${fmw_wls::tmp_dir}/${wls_template} -log=${fmw_wls::tmp_dir}/wls.log -log_priority=info",
    ...
    exec{ 'Install WLS':
      command => "${java_home_dir}/bin/java -d64 -Xmx1024m -Djava.io.tmpdir=${fmw_wls::tmp_dir} -jar ${source_file} -silent -responseFile ${fmw_wls::tmp_dir}/${wls_template} -invPtrLoc ${fmw_wls::orainst_dir}/oraInst.loc",
